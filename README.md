...
1. Copy `template/article/` into `assets/papers/your-topic/`.
2. Create a thin wrapper `articles/04-yourname.tex` that calls `\articleheader`
   and `\input{}`s your section files.
3. If you have images, add `{assets/papers/your-topic/}` to `\graphicspath`
   in `cmj.sty`.
...
- Images go in `assets/papers/<topic>/` and `\graphicspath` is updated in
